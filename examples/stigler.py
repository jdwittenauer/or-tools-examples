...
def main():
    ...
    data, nutrients = get_data()

    # Instantiate a Glop solver.
    solver = pywraplp.Solver('Stigler', pywraplp.Solver.GLOP_LINEAR_PROGRAMMING)
    ...
    if status == solver.OPTIMAL:
        ...
        price = 0
        nutrients = [0] * len(nutrients)
        for i in range(0, len(data)):
            price += food[i].solution_value()

            for nutrient in range(0, len(nutrients)):
                nutrients[nutrient] += data[i][nutrient + 3] * food[i].solution_value()

            if food[i].solution_value() > 0:
                print('{0:s} = {1:f}'.format(data[i][0], food[i].solution_value()))

        print('Optimal annual price: ${0:2f}'.format(365 * price))
    else:
